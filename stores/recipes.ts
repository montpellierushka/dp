...
export const useRecipesStore = defineStore('recipes', {
  ...
  getters: {
    ...
    getRecipesByCountry: (state) => (country: string) => {
      return state.recipes.filter(recipe => recipe.country === country)
    },
    
    getRecipesByTag: (state) => (tag: string) => {
  ...
  actions: {
    ...
    addReview(recipeId: number, review: Omit<Review, 'id' | 'recipeId' | 'date' | 'likes' | 'isLiked'>) {
    ...
    updateReview(recipeId: number, reviewId: number, updatedReview: Partial<Review>) {
      const recipe = this.recipes.find(r => r.id === recipeId)
      if (recipe?.reviews) {
    ...
    toggleLike(recipeId: number, reviewId: number) {
      const recipe = this.recipes.find(r => r.id === recipeId)
      if (recipe?.reviews) {
